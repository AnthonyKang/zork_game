...
int main(){
	...
	while(!gameOver){
		...
		while(in_room){
			...
			vector<Container>::iterator currRoomContTriggIt = currRoomContainer.begin();
			for(currRoomContTriggIt = currRoomContainer.begin(); currRoomContTriggIt != currRoomContainer.end(); currRoomContTriggIt++){
				// for each container, get all the triggers
				vector<Trigger> currRoomContTrigg = currRoomContTriggIt->getTrigger();
				vector<Trigger>::iterator ContTriggIt = currRoomContTrigg.begin();
				// iterate through each trigger
				for(ContTriggIt = currRoomContTrigg.begin(); ContTriggIt != currRoomContTrigg.end(); ContTriggIt++){
					Condition contCondition = ContTriggIt->getCondition();
					string has = contCondition.has;
					vector<string> contTriggAction = ContTriggIt->getAction();
					// get items in container
					vector<string> contItem = currRoomContTriggIt->getItem();
					vector<string>::iterator contItemIt = contItem.begin();
					// check if container contains item that matches condition
					if(has == "yes"){
						for(contItemIt = contItem.begin(); contItemIt != contItem.end(); contItemIt++){
							if(*contItemIt == contCondition.object){
								cout << ContTriggIt->getPrint() << endl;
								// iterate through each action
								vector<string> actionIt = contTriggAction->begin();
								//updateContainer(*currRoomContTriggIt, contTriggAction);
							}
						}
					}
				}
